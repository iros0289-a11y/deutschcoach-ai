    ...
    <Screen contentContainerStyle={styles.container}>
      <InfoCard>
        ...
        <Text style={styles.body}>
          Starte am besten mit {skillSummaries[weakestSkill].title}. Dort ist aktuell noch das meiste Potenzial.
        </Text>
        <PrimaryButton href={`/${weakestSkill}`} label={`Weiter mit ${skillSummaries[weakestSkill].title}`} />
      </InfoCard>
      ...
      <InfoCard>
        ...
        <Text style={styles.body}>
          {errorItems.length > 0
            ...
            : "Keine offenen Fehler. Nutze jetzt eine Modellprüfung oder trainiere gezielt eine Fertigkeit."}
        </Text>
        <PrimaryButton
          href={errorItems.length > 0 ? "/trainer" : "/exams"}
          label={errorItems.length > 0 ? "Zum Fehlertrainer" : "Zur Modellprüfung"}
        />
      </InfoCard>
      ...
      <View style={styles.grid}>
        {learningOrder.map((skill) => (
          <InfoCard key={skill}>
            ...
            <Text style={styles.cardText}>{skillSummaries[skill].subtitle}</Text>
